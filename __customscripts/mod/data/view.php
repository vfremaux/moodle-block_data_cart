...
///////////////////////////////////////////////////////////////////////////
    ...
    // require_once(__DIR__ . '/../../config.php'); // Customscripts.
    require_once($CFG->dirroot . '/mod/data/locallib.php');
    require_once($CFG->libdir . '/rsslib.php');
    require_once($CFG->dirroot . '/customscripts/mod/data/locallib.php');
...
/// Check further parameters that set browsing preferences
    ...
    if (!empty($advanced)) {
        ...
        $observed = clone($data);
        unset($observed->singletemplate);
        unset($observed->listtemplate);
        unset($observed->addtemplate);
        unset($observed->rsstemplate);
        unset($observed->csstemplate);
        unset($observed->asearchtemplate);
        // CHANGE+
        // Deroute this call to a modified library to extend array fields
        // list($search_array, $search) = data_build_search_array($data, $paging, $search_array);
        include_once($CFG->dirroot.'/customscripts/mod/data/locallib.php');
        list($search_array, $search) = data_build_search_array_multidim($data, $paging, $search_array);
        // CHANGE-
...
// Initialize $PAGE, compute blocks
    ...
    $PAGE->add_body_class('customscript');
...
if ($showactivity) {
    ...
    if ($mode == 'asearch') {
        $maxcount = 0;
        data_print_preference_form_multidim($data, $perpage, $search, $sort, $order, $search_array, $advanced, $mode);
